...
function check($bookid){
    ...
    $info = $_SESSION['userinfo'];
    $uid = $info['id'];
    $query = "SELECT * FROM BOOKS WHERE id = ANY(SELECT books_id FROM issue WHERE `contact_id` = $uid AND `books_id`=$bookid); ";
...
<table class="table" border="1">
    ...
    foreach($result as $record){
        ...
        if(check($record['id'])==True){
            echo '<td>'.'<a href="return.php?id='.$record['id'].'">Return</a>'.'</td>'; 
        }else{
            echo '<td>'.'<a href="issuse.php?id='.$record['id'].'">Issue</a>'.'</td>';
